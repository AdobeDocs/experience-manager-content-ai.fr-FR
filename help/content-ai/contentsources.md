---
title: Configurer et gérer vos sources d’IA dédiée au contenu
...
1. Connectez-vous à [Cloud Manager](https://my.cloudmanager.adobe.com/) et sélectionnez votre programme.

   ![Accueil Cloud Manager affichant la carte du programme](../assets/content-ai-onboarding-step-1.png)

1. Dans la **[!UICONTROL vue d’ensemble du programme]**, accédez à la section **[!UICONTROL Environnements]** et sélectionnez l’environnement à configurer.

   ![Vue d’ensemble du programme avec un environnement de production mis en évidence](../assets/content-ai-onboarding-step-2.png)

1. Sur la page des détails de l’environnement, sélectionnez l’onglet **[!UICONTROL Configuration de l’IA dédiée au contenu]**.

   ![Page de détails de l’environnement avec l’onglet Configuration de l’IA dédiée au contenu en évidence](../assets/content-ai-onboarding-step-3.png)

## Étape 2 : créer une source d’IA dédiée au contenu {#create-source}

Une source de contenu définit le site web que l’IA dédiée au contenu explore et indexe.

1. Dans l’onglet **[!UICONTROL Configuration de l’IA dédiée au contenu]**, sélectionnez **[!UICONTROL Créer une source]**.

   ![Onglet Configuration de l’IA dédiée au contenu affichant le bouton Créer une source](../assets/content-ai-onboarding-step-4.png)

1. Dans la boîte de dialogue **[!UICONTROL Créer/ajouter une nouvelle source d’IA dédiée au contenu]**, renseignez les champs suivants :

   | Champ | Description |
   | --- | --- |
   | **[!UICONTROL Nom de la configuration d’IA dédiée au contenu]** | Identifiant unique de cette source (par exemple, `my-site-index`). Impossible à changer après la création. |
   | **[!UICONTROL Description]** | *(Facultatif)* Brève description de la source de contenu. |
   | **[!UICONTROL Adresse du site web]** | URL racine du site web à explorer (par exemple, `https://www.example.com/`). |
   | **[!UICONTROL URL à exclure]** | *(Facultatif)* Modèles d’URL à ignorer lors de l’exploration. |
   | **[!UICONTROL Fréquence d’actualisation]** | Fréquence à laquelle l’IA dédiée au contenu explore à nouveau la source : hebdomadaire, quotidienne, 4 fois par jour, toutes les 60 minutes ou toutes les 15 minutes. |

   ![Boîte de dialogue Créer une source d’IA dédiée au contenu avec les champs de nom et d’adresse du site web renseignés et le bouton Créer une source en évidence](../assets/content-ai-onboarding-step-5-0.png)
...
1. Dans la liste des sources, sélectionnez l’icône **Plus d’actions** (…) en regard de votre source, puis **[!UICONTROL Déclencher l’acquisition]**.

   ![Liste des sources de l’IA dédiée au contenu avec le menu Plus d’actions ouvert et Déclencher l’acquisition mis en évidence](../assets/content-ai-onboarding-step-7.png)

1. Dans la boîte de dialogue **[!UICONTROL Déclencher l’acquisition]**, passez en revue les détails de la source (**[!UICONTROL Source de contenu]**, **[!UICONTROL Dernière exécution]** et **[!UICONTROL Prochaine exécution planifiée]**) et sélectionnez **[!UICONTROL Déclencher]**.

   ![Boîte de dialogue Déclencher l’acquisition](../assets/content-ai-onboarding-step-8.png)
...
![Liste des sources de contenu affichant le statut de Indexation](../assets/content-ai-onboarding-step-9.png)

![Liste des sources de contenu affichant le statut Disponible](../assets/content-ai-onboarding-step-10.png)

Attendez que le statut devienne **Disponible** avant de rechercher l’index ou de tester l’API.

## Étape 5 : rechercher du contenu indexé {#search-content}
...
1. Saisissez une requête dans le champ de recherche. Les résultats affichent une liste d’éléments correspondants avec un score et un type de contenu correspondants (par exemple, **PAGE** ou **PDF**). La sélection d’un résultat ouvre un aperçu à droite.
   ...
   ![Panneau de recherche avec une requête, les résultats correspondants avec des scores de correspondance et un volet d’aperçu du premier résultat](../assets/content-ai-onboarding-step-14.png)

## Modifier ou supprimer une source {#modify-source}

### Modification d’une source {#modify}

Pour mettre à jour une configuration source après sa création :

1. Dans la liste des sources, sélectionnez l’icône **Plus d’actions** (…) en regard de la source, puis **[!UICONTROL Modifier]**.

   ![Liste des sources de contenu avec le menu Plus d’actions ouvert et Modifier mis en surbrillance](../assets/content-ai-onboarding-step-11.png)
...
* [Configurer un projet Adobe Developer Console](setup-adc-project.md) : créez le projet ADC et les informations d’identification dont vous avez besoin pour appeler l’API
* [Référence de l’API d’IA dédiée au contenu](https://developer.adobe.com/experience-cloud/experience-manager-apis/api/experimental/contentai/) : interrogez votre contenu indexé à l’aide de points d’entrée de recherche sémantiques, en texte intégral ou hybride.

## Résolution des problèmes {#troubleshooting}

* **La source reste dans le statut [!UICONTROL Indexation] pendant une période prolongée.** Réessayez l’acquisition à partir du menu (…). Si le statut ne progresse pas après une seconde exécution, vérifiez que l’**[!UICONTROL adresse du site web]** est accessible au public et que les modèles **[!UICONTROL URL à exclure]** ne filtrent pas chaque page.
* **La source revient au statut [!UICONTROL Nouveau] après une exécution.** Le robot d’exploration n’a pu récupérer aucune page à partir de l’URL racine configurée. Vérifiez que l’URL répond avec `200 OK` et que le site ne bloque pas les requêtes automatisées.
* **[!UICONTROL La recherche] ne renvoie aucun résultat pour une source [!UICONTROL Disponible].** Indexation réussie, mais aucun contenu ne correspondait à la requête. Essayez une requête plus large ou vérifiez que les URL explorées incluent les pages attendues.
